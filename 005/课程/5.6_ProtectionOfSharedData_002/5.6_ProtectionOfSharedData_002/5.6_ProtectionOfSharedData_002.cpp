...
class Point
{
    ...
    //2.注意，这里不能把下面这个友元函数变成常函数。因为这个函数不是Point类的成员函数。不能将一个非成员函数变成一个带const修饰的函数。（会有这样做的想法是因为我想看看const能否区分出对引用的改变也可能改变成员值）
...
class Line
{
    ...
    void changePointValue() const
    {
        ...
        */
        ...
        //4.而下面这行代码是可以通过编译的。也就是说常函数只会避免函数修改本类的成员的值（在这里就是指针指向的地址），而不会管指针所指向的对象的值是否在函数体内被改变。（前提是函数中没有涉及常指针成员）
        (*p).testForLine++;
