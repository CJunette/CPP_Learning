// 4.8_Union_002.cpp.cpp : This file contains the 'main' function. Program execution begins and ends there.
//Example
//union通常被植入class中使用。
...
class ExamInfo
{
    ...
    //这里当我把enum的这行代码写成“enum mode {GRADE, PASS, PERCENT}”时，出现了报错（mode is not a nonstatic data member or base class of class）。
    //将鼠标移到代码错误时候的上面的mode上，会显示mode是一个enum类型的类。而实际上，在这里我们需要的是一个mode类的对象。
    //这种“{}”在前，名称在后的写法。其实就相当于先写了“mode {GRADE, PASS, PERCENT}”，然后写“mode mode”创建了一个对象。（注意现实中不要让类名和变量名相同）
    //而在修改之后我们再看代码就会发现,mode有了“field”的字样，说明它现在是一个对象成员。
