...
int main()
{
    ...
    //4.这里提到的从基类向派生类的转换是指指针和引用的转换。
    {
        ...
        //5.另外，即使是基类指针指向了派生类对象的中间位置，在将基类指针转换回派生类指针时，指针会自动回到对象的开头位置。
        //8.这就让我更不理解为什么虚基类的指针不能转成派生类的指针了。
        Derived1 d1;
        d1.print();
        Base2 *pb2 = &d1;
        Derived1 *pd12 = static_cast<Derived1 *>(pb2);

        //7.书上提到“指针和引用的转换，只涉及创建新的指针和引用，不涉及创建新的对象”。但下面这个做法是可行的。
        //7.下面做法的实质是，用pb1所指向的对象为参数，执行Derived1的复制构造函数。
        Derived1 d1 = *static_cast<Derived1 *>(pb11);

        //8.从基类向派生类的转换时不能发生在虚基类上的。下面的代码会显示报错。
        //8.书上p302也给出了一些解释。我认为可能的原因是，从派生类向基类转化可以发生的原因是派生类本身记录了派生类对象的初始位置到虚基类位置的指针位移，
        //8.派生类通过直接基类向虚基类转化可以发生的原因可能是通过访问直接基类中的隐含指针，指针能够从直接基类位置转移到虚基类位置；
        //8.而上述两个步骤对于已经转移到虚基类位置的指针而言都是不可逆的。（虽然我觉得在知道派生类及各级基类大小的情况下，想要逆向移动指针也不是不可能的。所以我也不知道为什么这点实现不了？）
        /*
    ...
    {
        //9.当转换需要经过void指针时，即使转换前后的指针类型是相同的，也有可能出现错误。
        Derived1 d1;
        d1.print();
        //9.这里Base2的指针pb2指向的是对象d1的中间位置。
        Base2 *pb2 = &d1;
        pb2->print();
        //9.转换为void类型的指针pv后，pv同样指向的是对象d1的中间位置。
        void *pv = static_cast<void *>(pb2);
        cout << pv << endl;
        //9.再次将pv转换会Derived类型的指针时，本来应该将指针移到对象的开头位置的，但因为是从void类型的指针出发进行转换，因此指针仍然在对象的中间位置。
        Derived1 *pd1 = static_cast<Derived1 *>(pv);
        pd1->print();
        //9.这也就导致了对对象内部成员的访问出现了巨大的问题。
        cout << pd1->d1 << endl;
    }
}
